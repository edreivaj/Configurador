...
function ensureVRModal(){
  ...
  if (!overlay){
    ...
    overlay.addEventListener('click', (e)=>{ if (e.target === overlay) closeVRModal(); });
...
function openVRModal(){
  ...
  if (iframe) iframe.src = VR_URL; // siempre carga/reinicia
...
const steps = [
  ...
  { label:"¿Cuántas plantas?", key:"plantas", onlyIf:{ habitaciones:"3" },
    options: s => {
...
function renderStep(step){
  ...
  if (steps[step]?.onlyIf){
    ...
    if (!show) return renderStep(step+1);
  }
