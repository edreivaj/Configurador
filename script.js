...
// === VR Modal helpers ===
const VR_URL = "https://panorama.homestyler.com/v2?m=np&id=gfSp4hWDtRDKuWFtKjYSn4&locale=es_ES";
function ensureVRModal(){
  let overlay = document.getElementById('vrModalOverlay');
  if (!overlay){
    overlay = document.createElement('div');
    overlay.id = 'vrModalOverlay';
    overlay.className = 'modal-overlay';
    overlay.innerHTML = `
      <div class="modal-card" role="dialog" aria-label="Recorrido virtual">
        <button class="modal-close" id="vrModalClose" aria-label="Cerrar">×</button>
        <iframe id="vrIframe" class="modal-iframe" src="" allowfullscreen></iframe>
      </div>`;
    document.body.appendChild(overlay);
    // Solo cerrar con la X o con ESC (no cerrar por click en overlay para evitar cierres accidentales)
    document.getElementById('vrModalClose').addEventListener('click', closeVRModal);
    window.addEventListener('keydown', (e)=>{ if (e.key === 'Escape') closeVRModal(); });
  }
  return overlay;
}
function openVRModal(){
  const overlay = ensureVRModal();
  const iframe = document.getElementById('vrIframe');
  if (iframe) iframe.src = VR_URL; // recarga cada vez
  overlay.classList.add('open');
}
function closeVRModal(){
  const overlay = document.getElementById('vrModalOverlay');
  if (!overlay) return;
  const iframe = document.getElementById('vrIframe');
  if (iframe) iframe.src = ""; // liberar recursos
...
const container = document.getElementById('choices');
  ...
  // Botón: Recorrido virtual (Homestyler)
  const vrBtn = document.createElement('button');
  vrBtn.className = 'btn primary';
  vrBtn.textContent = 'Ver recorrido virtual';
  vrBtn.style.marginTop = '16px';
  vrBtn.onclick = openVRModal;
  summaryDiv.appendChild(vrBtn);
